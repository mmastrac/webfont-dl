...
function toDataURL(mimeType, buffer, url) {
  return "data:" + mimeType + ";base64," + base64.encode(buffer) + (url.hash ? url.hash : "");  
}
...
function writeFont(opts, src, font, body) {
  ...
  if (fmt && opts.modes[fmt.ext] == "data") {
    log.debug("Converting " + fmt.ext + " font " + src.url + " to data, mime-type '" + fmt.mime + "'");
    src.url = toDataURL(fmt.mime, body, parsed);
    promise.resolve();
